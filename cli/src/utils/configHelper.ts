...
export function initConfigFile(quiet: boolean = false) {
...
export function writeConfigFile(data: any) {
    ...
    try {
        ...
        Logger.error(`Failed to write config file 'aerocloud.json'`, (err as Error).message);
    }
}
...
export function runBuildCommandIfExists() {
    ...
    if (buildCommand && buildCommand.trim() !== '') {
        ...
        try {
            ...
            Logger.error('Error executing build command', (error as Error).message);
