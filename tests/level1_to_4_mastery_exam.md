# 🚀 AeroCloud Mastery Examination (Levels 1 - 4.5)
**Candidate:** Golu (Age 20)  
**Role:** Systems Architect & Cloud Engineer  
**Context:** AeroCloud Architecture (Node.js, TypeScript, Go, Dockerode, SQLite, Caddy/Proxy)

---

## 🎯 PHASE 1: Advanced Systems & Architecture (10 MCQs)

### 1. Docker Tar Stream & Dynamic Packaging
...
- **C)** Pumping an in-memory tar stream over the Docker socket avoids massive disk I/O thrashing and allows precise algorithmic filtering (like omitting `node_modules`), vastly reducing context transfer time.
- **D)** The Docker socket only accepts `application/x-tar` streams if they are compressed via gzip in real-time.

---

### 2. Go Port Scanner & Stdin Search
...
- **B)** Passing state via Stdin to a highly concurrent Go routine and doing binary search guarantees near-zero kernel context switches and prevents ephemeral port collision races during parallel scale-ups.
...
### 3. Reverse Proxy & Host Header Routing
...
- **B)** The proxy extracts the `Host` header from the HTTP request, queries SQLite for the corresponding internal container IP/port mapped to `subdomain`, and proxies the TCP stream directly to that target.
...
- **B)** Appending writes to a Write-Ahead Log without blocking concurrent readers, while `synchronous=NORMAL` reduces `fsync()` calls to the disk, balancing corruption safety with high write throughput.
...
- **A)** GCM uses a 16-byte Auth Tag to mathematically prove the ciphertext has not been tampered with (Authenticated Encryption), preventing padding oracle attacks and silent data corruption inherent in CBC/ECB.
...
- **B)** Standard string comparisons short-circuit and return `false` at the first mismatched character, allowing an attacker to brute-force the HMAC signature byte-by-byte by observing response timing variations.
...
### 7. Streaming SizeGuard & Decompression Bomb DoS
...
- **C)** The `Transform` stream intercepts the incoming data pipe in real-time, incrementing a byte counter per chunk. If the counter exceeds the hard limit, it forcefully destroys the socket connection *before* the data reaches the disk or memory limits.
- **D)** It relies on Docker's `--storage-opt` flag to prevent the container from growing beyond the configured filesystem size.

---

### 8. Linux cgroups & Memory Swap Sandboxing
...
- **B)** The container is hard-capped at 512MB of physical RAM and 512MB of disk swap (Total: 1GB). If the application allocates beyond 1GB, the kernel's OOM Killer forcefully sends a `SIGKILL` to the container process.
...
### 9. 1.5s Boot Liveness Latch (Anti-Ghost Deployments)
Why does AeroCloud implement a 1.5s grace period with a subsequent `container.inspect()` check after `await container.start()`?
- **A)** Docker API's `start()` method returns asynchronously before the container OS fully boots. A container might crash immediately (e.g., missing start script, missing env vars). The latch waits and inspects the `State.Running` flag to prevent routing traffic to a dead "ghost" deployment.
...
### 10. Zero-Downtime Container Swaps & Teardown Order
...
- **D)** 1. Start Container N+1. $\rightarrow$ 2. Verify Liveness Latch (Wait 1.5s, Inspect). $\rightarrow$ 3. Update SQLite routing table to point to N+1. $\rightarrow$ 4. Teardown/Remove Container N.

---

## 🎯 PHASE 2: Scenario & Architectural Trade-off Deep Dives

### Question 1: Trade-off Analysis — Native `fetch` vs. GitHub `Octokit` SDK
Analyze the architectural trade-offs between using Node's native `fetch` versus the GitHub `Octokit` SDK. Discuss dependency bloat, stream piping efficiency (SizeGuard integration), and network protocol transparency. Which is superior for AeroCloud's specific pipeline and why?

### Question 2: Disaster Recovery Scenario — The Decompression Bomb & Network Exhaustion
A malicious user pushes a heavily compressed 2GB tarball to a linked GitHub repository, attempting to trigger a Decompression Bomb and network exhaustion attack. Trace step-by-step how AeroCloud's multi-layered defense (HMAC $\rightarrow$ 100ms ACK $\rightarrow$ SizeGuard $\rightarrow$ cgroups) neutralizes the attack before it can compromise the host machine.

### Question 3: Concurrency & State Machine — Local ZIP Upload (`aerocloud deploy`) vs. Git Webhook Push
What happens at the database and filesystem levels when User A runs `aerocloud deploy` locally while GitHub simultaneously sends a webhook push for the very same project? Explain how the state machine reconciles this concurrency. How do you prevent corrupting the build context or creating ghost containers?

### Question 4: Port Binding Physics — Single-Port vs. Multi-Port Binding
During development, binding both `3000/tcp` and `8080/tcp` to the same host port caused a fatal Docker kernel bridge collision. Explain the physics behind this failure. Differentiate clearly between the *Container Internal Port*, the *Host Mapped Port*, and the *Proxy Listener Port*.

### Question 5: Architectural Evolution — What breaks at 10,000 requests/sec?
As AeroCloud scales beyond Level 4, analyze the bottlenecks of the current single-node SQLite + Node.js Proxy architecture. Detail the exact failure points when hitting 10,000 requests per second. Explain the Level 5 migration path to Rust (`Tokio`/`Axum`) + Postgres/Redis, justifying the architectural shift.
